...
async function run() {
  try {
    ...
    //DELETE API (delete interested event by id)
    ...
    //DELETE API (delete joined event by id)
    app.delete('/joinedEvents/:id', async(req, res) =>{
      const id = req.params.id;
      const query = {_id: ObjectId(id)};
      const result = await joinedEventsCollection.deleteOne(query);

      console.log('deleting joined event with id: ', result);
...
// https://safe-citadel-76278.herokuapp.com/